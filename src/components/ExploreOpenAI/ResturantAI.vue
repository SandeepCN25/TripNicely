  ...
  <v-container fluid class="bg-white fill-height">
    <div class="scroll-container mt-2">
      <v-row v-if="loading" class="width-100%">
        <p class="mt-8 ml-8">Getting Data...</p>
        <v-col v-for="n in 6" :key="n" cols="12">
          <v-skeleton-loader class="mx-auto my-3 w-100%" type="card" />
        </v-col>
      </v-row>
      <!-- Hotel Cards -->
      <v-row v-else>
        <v-col v-for="(hotel, index) in hotels" :key="index" cols="12">
          <v-card class="text-center my-2" outlined>
            <v-card-title
              class="headline mt-2 text-amber-darken-2 text-h5 font-weight-bold"
            ...
            <v-btn small class="mb-4 bg-amber-lighten-1">see more</v-btn>
          </v-card>
        </v-col>
      </v-row>
    </div>
    ...
    const res = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "user",
            content: `Give me a list of hotels in ${city} with their type, rating, location.`,
          },
        ],
      }),
    });
